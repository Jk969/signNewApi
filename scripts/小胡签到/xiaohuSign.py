...
CACHE_FILE = Path(__file__).parent / '.xiaohumini.site.json'
...
def main():
    ...
    if not accounts:
        ...
        send_push_plus('小胡AI签到-失败', '未找到有效的账号配置')
        return
    ...
    title = f'{status_emoji} 小胡AI签到 {success_count}成功 {already_count}已签 {fail_count}失败'
